...
protocol ResourceLoaderOwner: AnyObject {
...
final class ResourceLoaderDelegate: NSObject, AVAssetResourceLoaderDelegate, URLSessionDelegate, URLSessionDataDelegate, URLSessionTaskDelegate {
    ...
    private weak var owner: ResourceLoaderOwner?
    
    // MARK: Init
    init(url: URL, saveFilePath: String, owner: ResourceLoaderOwner?) {
        self.url = url
    ...
    deinit {
        print("🍎 ResourceLoaderDelegate - deinit")
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            sSelf.owner?.playerItem(didDownloadBytesSoFar: sSelf.fileHandle.fileSize, outOf: Int(dataTask.countOfBytesExpectedToReceive))
        }
    }
    ...
    private func downloadComplete() {
        ...
        DispatchQueue.main.async { [weak self] in
            guard let sSelf = self else { return }
            sSelf.owner?.playerItem(didFinishDownloadingFileAt: sSelf.saveFilePath)
        }
    }
    ...
    private func downloadFailed(with error: Error) {
        ...
        DispatchQueue.main.async { [weak self] in
            guard let sSelf = self else { return }
            sSelf.owner?.playerItem(downloadingFailedWith: error)
        }
    }
